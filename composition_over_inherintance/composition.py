...
A reffer from https://github.com/ArjanCodes/2021-composition-vs-inheritance/blob/main/with_composition.py
"""
...
def main() -> None:
    ...
    diego_contract = FreelancerContract(pay_rate=20, hours_worked=44, vat_number="Awesome vat number")
    diego_comission = ContractCommission(commission=200, contracts_landed=30)
    diego = Employee(
        name="Diego", id=123456789, contract=diego_contract, commission=diego_comission
    )
    print(
        f"{diego.name} worked as a freelancer with comission for {diego_contract.hours_worked} hours, "
        f"earned ${diego.compute_pay()} and landed {diego_comission.contracts_landed} contracts."
    )
