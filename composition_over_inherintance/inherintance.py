...
A reffer from https://www.youtube.com/watch?v=0mcP8ZpUR38&t=1128s
"""
...
def main() -> None:
    ...
    diego = HourlyEmployeeWithCommission(
        name="Diego", id=123467, pay_rate=50, hours_worked=100, contracts_landed=20
    )
    print(
        f"{diego.name} worked for {diego.hours_worked} hours and earned ${diego.compute_pay()} "
        f"with {diego.contracts_landed} contracts landed"
    )

    jhon = FreelancerWithCommission(
        name="Jhon", id=123456789, pay_rate=20, hours_worked=44, vat_number="awesome vat number",
        commission=50, contracts_landed=30
    )
    print(
        f"{jhon.name} worked as a freelancer with comission for {jhon.hours_worked} "
        f"hours and earned ${jhon.compute_pay()} "
        f"with {diego.contracts_landed} contracts landed"
    )
